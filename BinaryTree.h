...
BinaryTree::size(const BinaryNode * subtree)
{
	...
	{
		sizesub = 1 + size(subtree->left_) + size(subtree->right_);
	}
	return sizesub;
}
...
BinaryTree::height(const BinaryNode * subtree)
{
	...
	else
	{
		...
		{
			l++;
			l += height(subtree->left_);
			return l;
		}

		BinaryNode * lheight = subtree->left_;
		BinaryNode * rheight = subtree->right_;

		long lh = height(lheight);
		long rh = height(rheight);
		if (lh > rh)
		{
			l++;
			l += height(lheight);
